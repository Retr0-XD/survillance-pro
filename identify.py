...
def identify():
	...
	while True:
		...
		for x,y,w,h in faces:
			...
			if labelslist[str(label[0])] == 'thamarai':
				if count == 1:
					subprocess.call('python bbb.py', shell=True)
					#os.system('start gnome-terminal -x py bb.py')
...
def maincall():
	...
	label = tk.Label(root, text="Select the below buttons ")
	...
	button2 = tk.Button(root, text="Trained Members", command=identify, height=2, width=20)
